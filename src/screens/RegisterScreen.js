...
export default function RegisterScreen({ navigation, onRegistered }) {
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      {step !== 'register' ? (
        <TextInput style={styles.input} placeholder="Telephone (+237...)" value={form.telephone}
      ...
      {step === 'verify' ? (
        <>
          ...
          <TouchableOpacity style={styles.buttonGreen} onPress={verifyAndRegister} disabled={loading}>
            {loading ? <ActivityIndicator color="#fff" /> : <Text style={styles.buttonText}>Verifier et creer le compte</Text>}
          </TouchableOpacity>
          <TouchableOpacity onPress={skipPhone}>
            <Text style={styles.link}>Continuer sans verifier le telephone</Text>
          </TouchableOpacity>
      ...
      {step === 'register' ? (
        <TouchableOpacity style={styles.buttonGreen} onPress={registerOnly} disabled={loading}>
          {loading ? <ActivityIndicator color="#fff" /> : <Text style={styles.buttonText}>Creer le compte</Text>}
        </TouchableOpacity>
      ) : null}
...
const styles = StyleSheet.create({
  ...
  buttonGreen: {
    backgroundColor: '#15803d', padding: 16, borderRadius: 8, alignItems: 'center', marginTop: 8,
  },
